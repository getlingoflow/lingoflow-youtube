...
export const I18N = {
  ...
  about_sync_api: {
    ...
    en: `Self-hosting a LingoFlow-worker data sync service`,
    zh_TW: `自建 lingoflow-worker 資料同步服務`,
    ja: `LingoFlow-worker データ同期サービスをセルフホストする`,
    ko: `LingoFlow-worker 데이터 동기화 서비스 자체 호스팅`,
  },
  about_api: {
  ...
  inject_js_helper: {
    zh: `预加载时注入，一个页面仅运行一次。内置全局对象 LingoFlow: {
      apiTranslate,
      apiDectect,
      apiSetting,
      apisMap,
    ...
    en: `Injected during preload, runs only once per page. Built-in global object LingoFlow: {
      apiTranslate,
      apiDectect,
      apiSetting,
      apisMap,
    ...
    zh_TW: `預先載入時注入，一個頁面僅運行一次。內建全域物件 LingoFlow: {
      apiTranslate,
      apiDectect,
      apiSetting,
      apisMap,
    ...
    ja: `プリロード時に注入され、ページごとに1回だけ実行されます。組み込みグローバルオブジェクト LingoFlow: {
      apiTranslate,
      apiDectect,
      apiSetting,
      apisMap,
    ...
    ko: `미리 로드 시 주입되며 페이지당 한 번만 실행됩니다. 내장 전역 객체 LingoFlow: {
      apiTranslate,
      apiDectect,
      apiSetting,
      apisMap,
  ...
  reg_niutrans: {
    zh: `获取小牛翻译密钥【LingoFlow专属新用户注册赠送300万字符】`,
    en: `Get NiuTrans APIKey [LingoFlow Exclusive New User Registration Free 3 Million Characters]`,
    zh_TW: `取得小牛翻譯金鑰【LingoFlow專屬新用戶註冊贈送 300 萬字元】`,
    ja: `NiuTrans APIキーを取得 [LingoFlow 専用 新規ユーザー登録で300万文字無料]`,
    ko: `NiuTrans API 키 받기 [LingoFlow 번역기 신규 사용자 등록 시 300만 자 무료 제공]`,
  },
  trigger_mode: {
    zh: `触发方式`,
    en: `Trigger Mode`,
  ...
  subtitle_data_is_ready: {
    zh: `字幕数据已准备就绪，请点击LingoFlow按钮加载`,
    en: `The subtitle data is ready, please click the LingoFlow button to load it`,
    zh_TW: `字幕資料已準備就緒，請點擊LingoFlow按鈕加載`,
    ja: `字幕データの準備ができました。LingoFlowボタンをクリックして読み込んでください`,
    ko: `자막 데이터가 준비되었습니다. LingoFlow 버튼을 클릭하여 로드하세요`,
  },
